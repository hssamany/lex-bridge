-- Stored procedure: batches orphaned line items into draft invoices per customer.
DELIMITER $$
...
BEGIN
    ...
    BEGIN
        ROLLBACK;
        DROP TEMPORARY TABLE IF EXISTS tmp_skipped_line_items;
        DROP TEMPORARY TABLE IF EXISTS tmp_pending_line_items;
        DROP TEMPORARY TABLE IF EXISTS tmp_customer_invoices;
        RESIGNAL;
    END;

    SET v_voucher_date = IFNULL(p_voucher_date, CURDATE());

    IF p_voucher_date IS NOT NULL AND p_voucher_date < DATE '2000-01-01' THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'voucher date must be on or after 2000-01-01';
    END IF;
    ...
    CREATE TEMPORARY TABLE tmp_skipped_line_items (
        line_item_id CHAR(36) NOT NULL PRIMARY KEY
    ) ENGINE = MEMORY;
    ...
    CREATE TEMPORARY TABLE tmp_pending_line_items (
        ...
        computed_line_total_net DECIMAL(18, 2) NULL,
        computed_line_total_gross DECIMAL(18, 2) NULL,
    ...
    SELECT COUNT(*) INTO v_pending_count FROM tmp_pending_line_items;
    ...
    ELSE
        ...
        IF NOT EXISTS (SELECT 1 FROM tmp_customer_invoices) THEN
            COMMIT;
        ...
        ELSE
            ...
            INNER JOIN tmp_pending_line_items t ON t.line_item_id = li.id
            INNER JOIN tmp_customer_invoices ci ON ci.customer_id = t.customer_id
            SET li.invoice_id = ci.invoice_id,
                li.line_order = CASE
    ...
    SELECT line_item_id FROM tmp_skipped_line_items;
